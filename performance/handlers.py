...
from .models import UserPerformance, UserPerformanceForModuleCompletion, UserQuizPerformance
...
def calculate_quiz_completion_status(user):
    all_quizubmissions_completed = QuizSubmission.objects.filter(
        user=user, is_completed=True
    ).exists()
    # all_questions_completed = Question.objects.filter(
    #     user=user, is_completed=True
    ...
    if all_quizubmissions_completed:
        return "Completed"
    else:
        return "Uncompleted"
...
def update_user_quiz_performance(user_id):
