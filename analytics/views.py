...
class AnalyticsViewSet(viewsets.ModelViewSet):
  ...
  queryset = Courses.objects.select_related('category','instructor').prefetch_related('courserating_set','coursereview_set')
  ...
  @action(detail=False, methods=['get'], url_name='student-completed-courses', url_path='student-completed-courses')
  def student_completed_course(self, request, *args, **kwargs):
    course_completed = Courses.objects.filter(enrollment__completion_status=True).select_related('category').annotate(number_of_students=Count('enrollment__student')).distinct()
    serializer = StudentCompletedCourseSerializer(course_completed, many=True)
    return response.Response(serializer.data)

  @action(detail=False, methods=['get'], url_name='most-viewed-course', url_path='most-viewed-course')
  def most_viewed_course(self, request, *args, **kwargs):
    return response.Response('Most viewed course')

  @action(detail=False, methods=['get'], url_name='most-purchased-course', url_path='most-purchased-course')
